...
describe('HOMEWORK', () => {
    ...
    it('Wait for element to be visible', () => {
        ...
        cy.get('#visibility_target').should('have.text', 'Click Me').and('be.visible').click();
        cy.get('.popover-header').should('be.visible').and('have.text','Can you see me?');
    ...
    it('el to be enabled', () => {        
        cy.get('#enabled_target').should('include.text','Disabled').and('not.be.enabled').and('have.css','background-color','rgb(255, 0, 57)');
        cy.get('#enabled_trigger').contains('button','Trigger').click();
        cy.get('#enabled_target').should('include.text','Enabled').and('have.css','background-color','rgb(63, 182, 24)').click();
    ...
    it('Wait for frame to be available and then switch to it', () => {
        ...
        cy.iframe().find('#inner_button').should('have.text','Inner Button').and('be.enabled').and('have.css','background-color','rgb(39, 128, 227)').click();
        cy.iframe().find('#inner_button').should('have.text','Clicked').and('be.enabled').and('have.css','background-color','rgb(55, 58, 60)');
    });
});
